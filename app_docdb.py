"""
DocumentDB 기반 센서 대시보드 애플리케이션

AWS DocumentDB에서 센서 데이터를 로드하여 대시보드로 시각화합니다.
"""
import os
import signal
...
if __name__ == '__main__':
    ...
    print(f"[시작] 포트 8050 정리 시작...")
    kill_port_process(8050)

    print(f"[대기] 포트 해제 대기 중...")
    time.sleep(3)  # 충분한 대기 시간 확보

    print(f"[초기화] DocumentDB 로더 생성...")
    loader = DocDBLoader()

    print(f"[초기화] 앱 생성...")
    app = create_app(loader, app_name="DocumentDB Sensor Dashboard", port=8050)

    print(f"[시작] Waitress 서버 시작 (http://0.0.0.0:8050)...")
    print(f"브라우저에서 http://localhost:8050 로 접속하세요")
