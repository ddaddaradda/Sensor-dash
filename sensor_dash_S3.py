...
aws_access_key_id = conf.S3BUCKET['ble_backup']['id']
aws_secret_access_key = conf.S3BUCKET['ble_backup']['key']
region_name = conf.S3BUCKET['ble_backup']['region']


# AWS S3 connection setup
s3_client = boto3.client('s3', aws_access_key_id=aws_access_key_id,
                      aws_secret_access_key=aws_secret_access_key, region_name=region_name)
ble_bucket = 'riderlog-driving-data'
lte_bucket = 'riderlog-driving-data-verlte'
...
def show_date(on):
    # BLE 센서 버전이라면
    if on == False:
        response = s3_client.list_objects_v2(Bucket=ble_bucket, Delimiter='/')
    ...
    # LTE 센서 버전이라면
        ...
        response = s3_client.list_objects_v2(Bucket=lte_bucket, Delimiter='/')
        if 'CommonPrefixes' in response:
            date_list = [prefix['Prefix'].rstrip('/') for prefix in response['CommonPrefixes']]
        else:
            date_list = []
...
def show_phonenum(date,on): 
    # BLE 센서 버전이라면
    if on == False:
        response = s3_client.list_objects_v2(Bucket=ble_bucket, Prefix=date)
        file_keys = [obj['Key'] for obj in response.get('Contents', []) if obj['Key'].strip() and obj['Key'].lower().endswith('.parquet')]
        phone_set = set()
    ...
    # LTE 센서 버전이라면
        ...
        response = s3_client.list_objects_v2(Bucket=lte_bucket, Prefix=date)
        file_keys = [obj['Key'] for obj in response.get('Contents', []) if obj['Key'].strip() and obj['Key'].lower().endswith('.parquet')]
        phone_set = set()
        for key in file_keys:
            parts = key.split('/')[-1].split('_')
...
def Load_BLEData(date,phone, sensor):
    ...
    response = s3_client.get_object(Bucket=ble_bucket, Key=object_key)
...
def Load_LTEData(date, phone, sensor):
    ...
    object_key = f"{date}/{sensor}_{phone}_{date}.parquet"
    response = s3_client.get_object(Bucket=lte_bucket, Key=object_key)
    parquet_content = response['Body'].read()
    # 가상 메모리의 내용을 BytesIO 객체로 변환
    parquet_buffer = BytesIO(parquet_content)
    # parquet 파일을 pandas DataFrame으로 읽기
...
# ------------------------------------------------------------------ 레이아웃 요소 ------------------------------------------------------------------
first_card = dbc.CardBody([html.H5("S3 data reading", className="card-title"),
...
def update_graph(n_clicks, value1, value2, value3,on):
    ...
    if on == False: # 센서가 BLE 버전일 때
        ...
        try:
            ...
            # mean_data = data.groupby(["DATE"]).mean()
